...
def display_timing_single(args):
    ...
    for model in args.model_list:
        input_file = (
            f"data/{args.bench_name}/model_answer/{model}.jsonl"
        )
        df_all = pd.read_json(input_file, lines=True)
        df = df_all[["model_id", "choices"]]
        df["choices"] = df["choices"].apply(lambda x: x[0]['proc_time'])
        df = df.explode("choices")
        df = df.rename(columns={'choices': 'timing', 'model_id': 'model'})
        dfs.append(df.groupby('model').agg({'timing': ['mean', percentile(0.1), percentile(0.9)]}))
    res_time = pd.concat(dfs, axis=0)
    # print(res_time)
    return res_time
